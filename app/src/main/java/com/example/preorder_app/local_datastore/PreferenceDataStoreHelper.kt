...
class PreferenceDataStoreHelper(context: Context):PreferenceDataStoreAPI {
    ...
    suspend fun getAllKeys(): Set<Preferences.Key<*>> {
        return dataSource.data.first().asMap().keys
    }

    suspend fun getAllValues(): List<String> {
        val preferences = dataSource.data.first()
        val allValues = mutableListOf<String>()

        for ((key, value) in preferences.asMap()) {
            // Convert 'value' to String and then add to 'allValues'
            val stringValue = value.toString()
            val keyValue = "$key: $stringValue"
